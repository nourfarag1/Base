...
class Spike(Object):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.image = self.spike["Blink (54x52)"][0]
        self.mask = pg.mask.from_surface(self.image)
        self.animation_count = 0
...
class Fruit(Object):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.image = self.fruit["Bananas"][0]
        self.mask = pg.mask.from_surface(self.image)
        self.animation_count = 0
...
def main(window):
    ...
    spike = Spike(250, HEIGHT - block_size - 500, 112, 96)

    fire = Fire(100, HEIGHT - block_size - 64, 16, 32)
    fire.on()

    checkpoint = Trophy(1000, HEIGHT - block_size - 120, 64, 96)
    ...
    objects = [*floor, Block(0, HEIGHT - block_size * 2, block_size),
               Block(block_size * 3, HEIGHT - block_size * 3, block_size),
                Block(block_size * 5, HEIGHT - block_size * 4, block_size),
                  flag, checkpoint, fire, spike, *fruits]
    ...
    while run:
        ...
        if pg.sprite.collide_mask(player, spike):
            spike.trigger()
            player.make_hit()
        ...
        if game_won:
            ...
            passed_text = font.render("Ta7eyaty, Noureldin Hatem", True, FONT_COLOR)
